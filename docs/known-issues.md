...
**Status:** Fixed 2026-09-13 (backend)
...
**Resolution 2026-09-13 (backend, PR #3, commit `2232360` — confirmed via the backend agent):** reviewer-visibility gap closed. `GET /admin/verifications?status=pending` returns the review queue (breeder name/email, association code/name, `registrationNumber`, `documentUrl`, `verificationStatus`: `unsubmitted`/`pending`/`approved`/`rejected` — no phone/CPF, same PII boundary as the rest of the API). `PATCH /admin/verifications/:breederId/:associationCode` with `{ status: 'approved'|'rejected', reason? }` records `reviewedBy`/`reviewedAt`/`rejectionReason`. Auth is an `ADMIN_EMAILS` env allowlist (not a DB role, a deliberate Phase-1 scale call) — Claudio is the reviewer for now. The full document lifecycle (upload → persist → review → approve/reject) now works end-to-end; **this clears the feature for launch.**

Same backend change also fixed a real interaction bug worth knowing about even though it required no FE change: `PATCH /breeders/:id` replaces the whole `associations[]` set (delete+reinsert) on every save, so previously any unrelated profile edit that re-sent the same associations array (which `EditProfileScreen` always does when `_associationsDirty`) would have silently wiped an admin's approval back to a blank review state. Now: re-sending the same `document_url` carries the review state forward untouched; a changed `document_url` correctly resets to `pending`; a removed document resets to `unsubmitted`. No request/response contract change on our side.

**Resolution 2026-09-13 (frontend, backend PR #6 commit `2bd0b8f`):** backend shipped the final contract — a top-level `associationVerified: boolean` on the breeder response (computed as `EXISTS` an approved association; present on `/auth/sync-breeder`, `/activate`, and `PATCH /breeders/:id`) plus per-association `verificationStatus`/`rejectionReason` on `associations[]` (returned by `/activate` and `PATCH /breeders/:id`, not `sync-breeder`). Deliberately kept separate from the existing `Breeder.verifiedBreeder` getter, which still gates Herd/Matches/Discover access via `profileStatus` alone — the two must not be conflated. FE wired up: `Breeder.associationVerified` (new badge on `profile_screen.dart`, "Associação Verificada"), `BreederAssociation.verificationStatus`/`rejectionReason` (shown per-row in `AssociationsPicker` and the read-only `BreederAssociationsCard` as "Em análise"/"Aprovado"/"Rejeitado: `<motivo>`"). Admin UI is still API-only (Postman), by design for Phase 1 scale.

**Not yet built, not launch-blocking:** no admin UI beyond the raw API — reviewed via direct calls for now. Open follow-up, not a blocker.
