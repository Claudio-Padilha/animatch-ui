...
**Not yet built, not launch-blocking:** no `verifiedBreeder` badge surfaced on the public API yet (the `CLAUDE.md` badge business rule), and no admin UI — the endpoint is API-only today, reviewed via direct calls. Both are open follow-ups.
